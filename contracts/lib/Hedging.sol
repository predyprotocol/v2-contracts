...
library Hedging {
     ...
     */
    function getEntry(PoolInfo storage _hedgeState, uint128 _spot)
        internal
        view
        returns (int128)
    {
        return
            (_hedgeState.latestEntrySize * int128(_spot)) /
            1e8 +
            _hedgeState.entry;
    }
    ...
    function complete(
        ...
        int128 totalLoss = netEntry -
            (int128(_spot) * _underlyingPositionDelta) /
            1e8;

        for (uint256 i = 0; i < 2; i++) {
            int128 loss = (int128(LiqMath.abs(_info.pools[i].latestEntry)) *
                totalLoss) / int128(totalEntry);

            _info.pools[i].entry =
                _info.pools[i].entry +
                _info.pools[i].latestEntry -
                loss;
