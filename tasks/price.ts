...
task('price', 'get trade price')
  ...
  .setAction(async ({ productId, tradeAmount }, hre) => {
    ...
    const futureTradePrice = await perpetualMarket.getTradePrice(0, tradeAmount)
    const sqeethTradePrice = await perpetualMarket.getTradePrice(1, tradeAmount)
    ...
    console.log(' Delta        : ', toUnscaled(futureTradePrice[1].mul(2).div(10000), 8).toLocaleString())
  })
