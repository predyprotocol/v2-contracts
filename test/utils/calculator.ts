//
// From https://github.com/opynfinance/squeeth-monorepo/blob/main/packages/hardhat/test/calculator.ts
//
...
function convertRawPriceToSqrtX96(rawPrice: BigNumber) {
  const sqrtX96Price = rawPrice
    .times(new BigNumber(2).pow(96 * 2))
    .div(1e18)
    .squareRoot()
  return sqrtX96Price
}
...
export function getSqrtPriceAndTickBySqueethPrice(price1e18: string | BigNumberish, wethIsToken0: boolean) {
  ...
  const newToken0Price = wethIsToken0 ? new BigNumber(1).div(humanReadablePrice).toString() : humanReadablePrice
