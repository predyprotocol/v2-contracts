...
describe('IndexPricer', function () {
  ...
  describe('calculatePrice', () => {
    it('reverts if productId is greater than 2', async function () {
      await expect(tester.verifyCalculatePrice(2, scaledBN(1000, 8))).to.be.revertedWith('NP')
    })
    ...
    describe('productId is 1', () => {
      ...
      it('calculate price when spot is $2,000', async function () {
        const price = await tester.verifyCalculatePrice(productId, scaledBN(2000, 8))

        expect(price).to.equal(scaledBN(400, 8))
      })

      it('calculate price when spot is $3,000', async function () {
        const price = await tester.verifyCalculatePrice(productId, scaledBN(3000, 8))

        expect(price).to.equal(scaledBN(900, 8))
      })

      it('calculate price when spot is $4,000', async function () {
        const price = await tester.verifyCalculatePrice(productId, scaledBN(4000, 8))

        expect(price).to.equal(scaledBN(1600, 8))
      })

      it('calculate price when spot is $10,000', async function () {
        const price = await tester.verifyCalculatePrice(productId, scaledBN(10000, 8))
    ...
    describe('productId is 0', () => {
      ...
      it('calculate price when spot is $1,000', async function () {
        const price = await tester.verifyCalculatePrice(productId, scaledBN(1000, 8))
  ...
  describe('calculateDelta', () => {
    it('reverts if productId is greater than 2', async function () {
      await expect(tester.verifyCalculateDelta(2, scaledBN(1000, 8))).to.be.revertedWith('NP')
    })
    ...
    describe('productId is 0', () => {
      ...
      it('calculate delta when spot is $2,000', async function () {
        const delta = await tester.verifyCalculateDelta(productId, scaledBN(2000, 8))

        expect(delta).to.equal(scaledBN(4, 7))
      })

      it('calculate delta when spot is $3,000', async function () {
        const delta = await tester.verifyCalculateDelta(productId, scaledBN(3000, 8))

        expect(delta).to.equal(scaledBN(6, 7))
      })

      it('calculate delta when spot is $4,000', async function () {
        const delta = await tester.verifyCalculateDelta(productId, scaledBN(4000, 8))

        expect(delta).to.equal(scaledBN(8, 7))
      })

      it('calculate delta when spot is $10,000', async function () {
        const delta = await tester.verifyCalculateDelta(productId, scaledBN(10000, 8))
    ...
    describe('productId is 1', () => {
      ...
      it('calculate delta when spot is $1,000', async function () {
        const delta = await tester.verifyCalculateDelta(productId, scaledBN(1000, 8))
  ...
  describe('calculateGamma', () => {
    it('reverts if productId is greater than 2', async function () {
      await expect(tester.verifyCalculateGamma(2)).to.be.revertedWith('NP')
    })

    it('gamma is 2', async function () {
      expect(await tester.verifyCalculateGamma(SQUEETH_PRODUCT_ID)).to.be.eq(20000)
    })

    it('gamma is 0', async function () {
      expect(await tester.verifyCalculateGamma(FUTURE_PRODUCT_ID)).to.be.eq(0)
    })
  })
})
